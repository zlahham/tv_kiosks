...
Please register for a developer account id needed and generate API keys for the following:
...
After you have acquired all 4 keys, you will have to save those keys locally in a way that the application can access them from the PATH.
...
After installing all the packages and libraries in the previous sections, you will now be able to run the application server and use TV Kiosk system.
...
There are a few files that you have to update once you are done with the Passenger + Nginx tutorial that was linked in Section B.1.

Assuming that the configuration file that you added is named as `/etc/nginx/conf.d/tv_kiosks.conf`, it should have the following directives and their parameters:
...
As of writing, there are no extra steps needed for our application to connect to Redis, it should work with the options that are set inside our source code at `tv_kiosks/config/cable.yml`.
...
We will be documenting some of the custom features in our application below. Here is the simplified code structure of the application:
